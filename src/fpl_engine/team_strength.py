...
def compute_team_strength(
    ...
    clip: tuple[float, float] = (0.75, 1.25),
) -> pd.DataFrame:
    """
    ...
    if matches_df is None or matches_df.empty:
        return pd.DataFrame(
            columns=["team", "atk_home", "atk_away", "def_home", "def_away"]
        )
    ...
    agg["def"] = (
        (max(1e-6, league_ga) / agg["ga"]).replace([np.inf, -np.inf], 1.0).clip(*clip)
    )

    atk = agg.pivot(index="team", columns="venue", values="atk").rename(
        columns={"home": "atk_home", "away": "atk_away"}
    )
    ddf = agg.pivot(index="team", columns="venue", values="def").rename(
        columns={"home": "def_home", "away": "def_away"}
    )
