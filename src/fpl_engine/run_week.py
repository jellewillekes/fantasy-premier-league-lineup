...
def compute_team_strengths() -> pd.DataFrame:
    ...
    strengths = compute_team_strength(
        matches, standings
    )  # team, atk_home, atk_away, def_home, def_away
    return strengths


def force_include_names(
    players_filtered: pd.DataFrame, players_full: pd.DataFrame, names: list[str]
) -> pd.DataFrame:
...
def ids_failing_minutes(
    players_full: pd.DataFrame, players_filtered: pd.DataFrame, names: list[str]
) -> list[int]:
    ...
    ids_ok = set(
        pd.to_numeric(players_filtered["id"], errors="coerce").dropna().astype(int)
    )
...
def ep_table(
    ...
    s_map = (
        strengths.set_index("team").to_dict(orient="index")
        if "team" in strengths.columns
        else {}
    )
    ...
    for _, p in players.iterrows():
        ...
        for gw in horizon_gws:
            ...
            per_fix_minutes = distribute_minutes_dgw(
                mins_est["E_min_if_start"],
                fixtures_in_gw=len(team_fx),
                is_gkp=(pos == "GKP"),
            )
            mean_sum = 0.0
            var_sum = 0.0
...
def main():
    ap = argparse.ArgumentParser(
        description="Weekly FPL run: EPs, wildcard best 15, lineup, and transfer suggestion."
    )
    ap.add_argument(
        "--horizon", type=int, default=4, help="Number of GWs to look ahead"
    )
    ap.add_argument(
        "--min_sixty",
        type=int,
        default=5,
        help="Minimum matches with ≥60 minutes to include a player",
    )
    ap.add_argument(
        "--use_wildcard",
        action="store_true",
        help="Build best-15 from scratch via wildcard",
    )
    ...
    players_full, players_filtered, fixtures = build_universe_pair(
        min_sixty=args.min_sixty
    )
    ...
    # Add horizon EPs
    players_union_ep = add_horizon_ep(
        players_union, ep_by_gw, gws
    )  # for printing/matching
    players_filtered_ep = add_horizon_ep(
        players_filtered, ep_by_gw, gws
    )  # optimization pool

    # --- Wildcard best 15 (from filtered pool) or fallback demo ---
    if args.use_wildcard:
        squad = solve_wildcard_from_ep(
            players_filtered_ep, ep_by_gw, gws, budget=args.budget, max_per_club=3
        )
        ...
        parts = [
            players_filtered_ep[players_filtered_ep["position_name"] == pos].nlargest(
                k, "h_ep"
            )
    ...
    if args.suggest_from_myteam:
        ...
        if not names:
            ...
            if flagged_ids:
                flagged_names = current[current["id"].isin(flagged_ids)][
                    "player_name"
                ].tolist()
                print(
                    f"[INFO] Players below minutes filter (prioritize replacing): {', '.join(flagged_names)}"
                )
    ...
    bank_after_wc = (
        max(0.0, args.budget - squad["now_cost"].sum()) if args.use_wildcard else 0.0
    )
