...
SCHEMA_PREFERRED = [
    "id", "player_name", "position_name", "club_name",
    "now_cost", "h_ep", "form", "points_per_game", "total_points",
    "selected_by_percent",
]

def _h_ep_for(pid: int, ep_by_gw: dict, gws: List[int]) -> float:
    return float(sum(ep_by_gw.get(int(pid), {}).get(gw, {}).get("mean", 0.0) for gw in gws))

def _ensure_h_ep_column(df: pd.DataFrame, ep_by_gw: dict, gws: List[int]) -> pd.DataFrame:
    out = df.copy()
...
def _compute_horizon_squad_ep(
    ...
    for gw in gws:
        xi, cap, vc, bench = best_xi_for_gw(squad, ep_by_gw, gw, risk_lambda=risk_lambda)
        gw_ep = float(xi["ep_mean"].sum())
        total += gw_ep
        details[gw] = {"xi": xi, "captain": cap, "vice": vc, "bench": bench, "ep_sum": gw_ep}
...
def _legal_candidate_pool(
    ...
    if "h_ep" in pool.columns:
        pool = pool.sort_values(["h_ep", "points_per_game"], ascending=False, na_position="last")
    else:
        pool = pool.sort_values(["points_per_game", "total_points"], ascending=False, na_position="last")
    return pool

def _club_ok_after_swap(
    out_row: pd.Series, in_row: pd.Series, club_counts: Dict[str, int], max_per_club: int
) -> bool:
...
def _safe_swap_row(new_squad: pd.DataFrame, out_id: int, cand: pd.Series) -> pd.DataFrame:
    """
...
def _try_single_transfer(
    ...
    if prefer_out_ids:
        ...
        out_rows.sort(key=lambda r: (0 if int(r.id) in prefer_set else 1, -float(getattr(r, "h_ep", 0.0))))

    best = {
        "gain": 0.0, "out": None, "in": None,
        "new_squad": None, "new_total_ep": base_ep,
        "base_total_ep": base_ep, "xi_by_gw": base_details,
    }
    ...
    for out_r in out_rows:
        ...
        pool = _legal_candidate_pool(uni, pos, exclude_ids=squad_ids).head(k_targets_per_out)
        ...
        for _, cand in pool.iterrows():
            ...
            if in_price > bank + out_price + 1e-9:
                continue
            # club cap
            if not _club_ok_after_swap(pd.Series(out_r._asdict()), cand, club_counts, max_per_club):
                continue
...
def suggest_best_transfer(
    ...
    while remaining_ft > 0:
        best_one = _try_single_transfer(
            squad, uni, bank=remaining_bank, gws=gws, ep_by_gw=ep_by_gw,
            max_per_club=max_per_club, k_targets_per_out=k_targets_per_out,
