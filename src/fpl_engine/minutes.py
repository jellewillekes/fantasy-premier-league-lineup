...
def estimate_minutes_prob(player_row: pd.Series, history_df: pd.DataFrame|None=None, short_rest: bool=False) -> dict:
    ...
    injury_penalty = 0.0
    if "doubt" in news or "late test" in news: injury_penalty = 0.15
    if "injury" in news or "ruled out" in news or "out" in news: injury_penalty = 0.40
    if "return" in news: injury_penalty = 0.10
    ...
    return {"P_start": p_start, "P60_if_start": p60_if_start,
            "E_min_if_start": float(e_min_if_start), "E_min_if_sub": float(e_min_if_sub)}

def distribute_minutes_dgw(base_minutes: float, fixtures_in_gw: int, is_gkp: bool=False) -> list[float]:
    if fixtures_in_gw <= 1:
