...
def best_xi_for_gw(squad_df: pd.DataFrame, ep_by_gw: dict[int, dict], gw: int, risk_lambda: float=0.0):
    df = squad_df.copy()
    df["ep_mean"] = df["id"].map(lambda i: ep_by_gw.get(i, {}).get(gw, {}).get("mean", 0.0))
    df["ep_var"]  = df["id"].map(lambda i: ep_by_gw.get(i, {}).get(gw, {}).get("var", 0.0))

    def pick(df,pos,k): return df[df["position_name"]==pos].nlargest(k, "ep_mean")
    ...
    bench_order = bench.sort_values("bench_score", ascending=False)["player_name"].tolist()
